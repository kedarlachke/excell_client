...
export default class File_Upload extends Component {
  ...
  async componentDidMount() {
    ...
    console.log("props in file upload");
    console.log(this.props);
    console.log("props in file upload");
    // When In Lead Edit Populate Data
    if (this.props.data || this.props.state) {
    ...
    if (!this.props.CLIENTID) {
      this.props.data || this.props.type
        ? null
        : this.props.showMsg(
            "Case details not save.\n Please save case details first",
            true
          );
      this.props.data || this.props.type ? null : this.props.gotoPreviousTab(1);
    }
  }
  Onclear() {
    this.setState({
  ...
  setCreateDocParams() {
    ...
    var entity = "";
    this.props.data
      ? (entityid = this.props.data.data.CID)
      : (entityid = this.props.Case_Id );
  ...
  async populateList() {
    ...
    if (!result) {
      ...
      if (result.data.searchUploadedDocuments != 0) {
        await this.setState({
          srno: result.data.searchUploadedDocuments[0].SRNO
        });
      }
      this.Onclear();
    ...
    return (
      <div>
        <FileUplaodList
          ...
          populateDocDetails={this.populateDocDetails}
  ...
  getUrl(id) {
    console.log("getURL");
    const url =  process.env.API_SERVER;
    return url + "/download?documentID=" + id + "";
  }

  navigateToLeadList() {
    if (this.props.data) {
      return this.props.history.push("/leads");
    } else if (this.props.isCust) {
  ...
  async updateDocs() {
    try {
     ...
     // const url = "http://81.4.102.11/excellinv";
      ...
      formData.append(
        "variables",
        JSON.stringify(this.setUpdateDocParams() || {})
      );
  ...
  DocCRUD_Ops() {
    if (this.state.tcode == "create") this.createDoc();
    else if (this.state.tcode == "update") this.updateDocs();
  }
      ...
      <div>
        <div className="ui one column grid">
          <div
            className="modal"
            style={{
              display: this.state.displayComp ? "flex" : "none",
          ...
          <div className="three column row">
            <div
              className={
                this.props.data
                  ? "one wide computer one wide tablet one wide mobile column"
                  : ""
              }
            />
            <div
              className={
                this.props.data
                  ? "fourteen wide computer fourteen wide tablet fourteen wide mobile column"
                  : "sixteen wide computer sixteen wide tablet sixteen wide mobile column"
              }
            >
                    ...
                    <div className="row">
                      <div className="ten wide column">
                        <div className="field">
                          <label
                            style={{
                              color: this.state.errorNotes ? "brown" : null
                            }}
                          >
                            {" "}
                            Notes
                          ...
                          <div className="ui right icon input">
                            <textarea
                              style={{
                                borderColor: this.state.errorNotes
                                  ? "brown"
                                  : null,
                                backgroundColor: this.state.errorNotes
                                  ? "#f3ece7"
                                  : null
                              }}
                              name="Notes"
                        ...
                        <div className="field">
                          {errorval ? (
                            <span id="errorspan">{this.state.errorNotes}</span>
                          ) : null}
                        </div>
                      </div>
                    </div>
                    <div className="row">
                      ...
                      <div className="five wide column">
                        <div className="field">
                          ...
                          <select
                            className=""
                            value={this.state.Document_Types}
                            onChange={e =>
                              this.setState({ Document_Types: e.target.value })
                            }
                          >
                            {/* <option value="">Select</option> */}
                            {this.state.Doc_Type_Arr.map(data =>
                              data.Documenttype.map((data, index) => (
                    ...
                    <div className="three column row">
                      <div className="ten wide column">
                        <button
                          ...
                          type="submit"
                          onClick={() => this.DocCRUD_Ops()}
                        >
                          Save
                        </button>
                        {this.props.data || this.props.type ? null : (
                          <button
                            className="ui primary button"
                            type="submit"
                            onClick={() =>
                              this.props.gotoCaseType(
                                true,
                                this.props.CLIENTID,
                                this.props.Case_Id
                              )
                        ...
                        <button
                          className="ui  button"
                          type="submit"
                          onClick={() => this.navigateToLeadList()}
                        >
                          Cancel
                        </button>
                      </div>
