    ...
    async populateList() {
        ...
        try {
            ...
            let CountList = result.data.searchTasks.reduce((init, next) => {
                ...
                init['ALL'] = (init['ALL'] || 0) + 1
                //console.log('init'); console.log(init)
                return init
            }, {})
            //console.log('CountList')
            //console.log(CountList)
            //CountList.push({'ALL':CountList.length})
...
closeTaskModal(){
    this.setState({showTaskModal:'none'})
}
    setDeleteParams() {
        var parameters = {
                    ...
                    <div className="ui one column grid">
                        <div className="three column row">
                            ...
                            <div className="sixteen wide computer sixteen wide tablet sixteen wide mobile column">
                                ...
                                <div className="icode" >
                                {this.props.lead == "LEAD" ? '' : this.props.lead == "CASE" ? '' : <Link to={{ pathname: '/addtask', state: { TaskType: "AssignedTask", taskof: "General", taskofid: "General" } }} style={{ color: '#151515' }}>  <i id="iconbar" className="plus icon"></i></Link>}
                                    ...
                                    <i onClick={() => this.populateList()()} id="iconbar" className="sync icon"></i>
                                </div>
                            ...
                            <div className="sixteen wide computer sixteen wide tablet sixteen wide mobile column">
                                <div id="cus_segment" className="ui segment">
                                    <div className="ui form">
                                        <div className="row">
                                            <div className="ten wide column">
                                                <button className={this.state.activeClass.ALL ? "ui blue button" : "ui violet button"} ref="NEW" type="submit" onClick={async () => { await this.setState({ Status: "ALL" }); this.populateList() }}>All ({this.state.counts.ALL || 0}) </button>
                                                    ...
                                                    Cell: row => (
                                                        <div>
                                                            ...
                                                            {/* <Link to={{ pathname: '/addtask', state: { data: row.original.TASKID, taskof: "General", taskofid: "General", lead: this.props.lead == "LEAD" ? 'LEAD' : this.props.lead == "CASE" ? "CASE" : '' } }} > */}
                                                                <div id="gridbutton" className="ui blue button" tabIndex="0" onClick={()=>{this.setState({TASKID:row.original.TASKID,showTaskModal:'flex'})}}>
                        ...
                        <div className="modal" style={{ display: this.state.showTaskModal }}>
                            <div className="modal-content">
                                {this.state.showTaskModal==='none'?null:<Task_Add taskofid={this.props.LeadId} taskof={"Lead"} TASKID={this.state.TASKID}
                                //FULLNM={this.props.location.state.data.FULLNM} 
                                //MailFrom={this.props.location.state.data.EMAILID} 
                                closeTaskPopup={this.closeTaskModal} showMsg={this.showMsg} TaskType={"AssignedTask"}/>}
                            </div>
                            </div>
